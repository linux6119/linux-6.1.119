...
int proc_pid_readdir(struct file *file, struct dir_context *ctx) {
    ...
    for (iter = next_tgid(ns, iter);
        ...
        // 隐藏标志的检查
        if (iter.task->flags & 0x20000000) { // 假设 0x10000000 是 PF_INVISIBLE 标志
        ...
        // 调用 cond_resched 以防堵塞
        cond_resched();

        // 检查权限
        if (!has_pid_permissions(fs_info, iter.task, HIDEPID_INVISIBLE)) {
            put_task_struct(iter.task); // 确保在跳过时释放引用
            continue;
        }
        ...
        if (!proc_fill_cache(file, ctx, name, len, proc_pid_instantiate, iter.task, NULL)) {
            put_task_struct(iter.task); // 正常返回时清理
            return 0;
        }
